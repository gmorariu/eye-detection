...
//  Pods
//
//  Created by George Morariu on 9/14/21.
//
...
class AWSS3Manager {
    ...
    func uploadVideo(videoUrl: URL, key: String, progress: progressBlock?, completion: completionBlock?) {
        //let fileName = self.getUniqueFileName(fileUrl: videoUrl)
        self.uploadfile(fileUrl: videoUrl, fileName: key, contenType: "video", progress: progress, completion: completion)
    }
    ...
    private func uploadfile(fileUrl: URL, fileName: String, contenType: String, progress: progressBlock?, completion: completionBlock?) {
        ...
        let awsTransferUtility = AWSS3TransferUtility.s3TransferUtility(forKey: "S3")!
